...
local function map(mode, shortcut, command, opts)
  ...
  if opts.remap ~= nil then opts.remap = true end
...
map({ 'n', 'v' }, '<leader>y', [["+y]], { desc = '[Y]ank to PRIMARY selection' })
nmap('<leader>Y', [["+Y]], { desc = '[Y]ank line to PRIMARY selection' })

-- Delete, sending it into the black-hole register.
map({ 'n', 'v' }, '<leader>d', [["_d]], { desc = '[D]elete into black-hole register' })
...
cmap('%%', "<c-r>=expand('%:h').'/'<cr>", { desc = 'Expand % to the current directory' })
...
map({ 'i', 'x' }, '<C-S>', '<Esc><Cmd>silent! update | redraw<CR>', { desc = '[S]ave and go to Normal mode' })
...
nmap('tc', '<Cmd>setlocal cursorline! cursorline?<CR>', { desc = "[T]oggle '[c]ursorline'" })
nmap('tC', '<Cmd>setlocal cursorcolumn! cursorcolumn?<CR>', { desc = "[T]oggle '[c]ursorcolumn'" })
nmap(
  'th',
  '<Cmd>let v:hlsearch = 1 - v:hlsearch | echo (v:hlsearch ? "  " : "no") . "hlsearch"<CR>',
  { desc = '[T]oggle search [h]ighlight' }
)
nmap('ti', '<Cmd>setlocal ignorecase! ignorecase?<CR>', { desc = "[T]oggle '[i]gnorecase'" })
nmap('tl', '<Cmd>setlocal list! list?<CR>', { desc = "[T]oggle '[l]ist'" })
nmap('tn', '<Cmd>setlocal number! number?<CR>', { desc = "[T]oggle '[n]umber'" })
nmap('tr', '<Cmd>setlocal relativenumber! relativenumber?<CR>', { desc = "[T]oggle '[r]elativenumber'" })
